import pytest
from mc4llm.example_calculators.bmi import BMIInput, WHO_BMI_CALCULATOR, ASIAN_BMI_CALCULATOR

def test_calculate_bmi_standard():
    # Test using SI units: weight in kg, height in m.
    input_data = BMIInput(weight=70, height=1.75)
    result = WHO_BMI_CALCULATOR.calculate(data=input_data)
...
def test_calculate_bmi_edge_cases():
    ...
    result_low = WHO_BMI_CALCULATOR.calculate(data=input_data_low)
    assert result_low.category == "Underweight"
    
    # Test very high BMI
    input_data_high = BMIInput(weight=150, height=1.75)
    result_high = WHO_BMI_CALCULATOR.calculate(data=input_data_high)
...
def test_bmi_guidelines_comparison():
    ...
    result_who = WHO_BMI_CALCULATOR.calculate(data=input_data)
    assert result_who.category == "Normal weight"
    
    # Test with Asian guidelines
    result_asian = ASIAN_BMI_CALCULATOR.calculate(data=input_data)
    ...
    result_borderline_who = WHO_BMI_CALCULATOR.calculate(data=input_data_borderline)
    result_borderline_asian = ASIAN_BMI_CALCULATOR.calculate(data=input_data_borderline)
